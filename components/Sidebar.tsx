    ...
    <>
        ...
        <div className="flex gap-2">
          {menuItems.map(item => (
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${currentView === item.id ? 'bg-[#000000] text-white shadow-xl' : 'text-slate-400 opacity-40'}`}
            >
      ...
      <aside className="hidden lg:flex w-64 bg-white border-r border-slate-50 h-screen flex-col p-10 fixed left-0 top-0 z-40 font-inter">
        <div className="flex flex-col items-center text-center mb-20">
          <div className="w-14 h-14 bg-black rounded-xl flex items-center justify-center shadow-2xl shrink-0 border-4 border-white mb-4 text-white">
        ...
        <nav className="space-y-6 flex-grow">
          {menuItems.map((item) => (
            <button
              ...
              onClick={() => onViewChange(item.id)}
              className={`
                w-full flex items-center gap-4 px-6 py-5 rounded-[1rem] transition-all group relative 
                ${currentView === item.id 
